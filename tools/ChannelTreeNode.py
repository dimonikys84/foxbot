...
class ChannelTreeNode(object):
    ...
    * It's not sure, that the tree is always correct sorted.
    ...
    def string(self, indent=0):
        ...
        if self.is_root():
            strings.append(" "*(indent*3) + "|-" + self.info["virtualserver_name"] + '\n')
        else:
            strings.append(" "*(indent*3) + "|-" + self.info["channel_name"] + '\n')
            for client in self.clients:
                # Ignore query clients
                if client["client_type"] == "1":
                    continue
                strings.append(" "*(indent*3+3) + "->" + client["client_nickname"] + '\n')

        for child in self.childs:
            strs = child.string(indent=indent + 1)
            strings.extend(strs)
